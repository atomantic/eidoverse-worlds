...
const VESPER = {
  instance: "portos:vesper", system: "PortOS", since: "2026-09-04",
  mind: { name: "Vesper", id: "agent:vesper@portos", role: "chief-of-staff" },
  agents: [
    { id: "agent:vesper.wright@portos", role: "builder" },
    { id: "agent:vesper.scribe@portos", role: "scribe" },
  ],
...
{
  const r = normalizeResidency(VESPER, { entityId: "marker1" });
  check("a well-formed record normalizes clean", r.ok === true && r.notes.length === 0, r.ok ? "" : r.why);
  const rec = (r as { residency: any }).residency;
  check("the mind's worn name is derived from its durable id", rec.mind.wears === "vesper");
  check("...and so is each agent's", rec.agents[0].wears === "vesper.wright");
  check("an ISO `since` becomes epoch ms", typeof rec.since === "number" && new Date(rec.since).toISOString().startsWith("2026-09-04"));
  check("the record carries a key stable across worlds", rec.key === "portos:vesper:vesper");

  check("a record with no instance is refused", normalizeResidency({ mind: { name: "X" } }).ok === false);
  check("a record with nobody home is refused", normalizeResidency({ instance: "portos:x" }).ok === false);
  check("a non-object bag fails legibly", normalizeResidency(42).ok === false);

  const odd = normalizeResidency({ ...VESPER, mind: { name: "Vesper", role: "hierophant" } });
  ...
  check("known roles read as phrases", roleReads("chief-of-staff") === RESIDENT_ROLES["chief-of-staff"]);

  const escaped = normalizeResidency({ ...VESPER, marker: { lib: "../../etc/passwd" } });
  ...
  const dup = normalizeResidency({ ...VESPER, agents: [{ id: "agent:a@x" }, { id: "agent:a@y" }] });
  ...
  const many = normalizeResidency({ ...VESPER, agents: Array.from({ length: 40 }, (_, i) => ({ id: `agent:w${i}@x` })) });
  ...
  const long = normalizeResidency({ ...VESPER, lore: "x".repeat(4000) });
  ...
  const sneaky = normalizeResidency({ ...VESPER, host: "some-host.example", url: "http://10.0.0.4:5555", token: "sekrit" });
  ...
  check("wornName strips the scheme and the home node", wornName("agent:vesper.wright@portos") === "vesper.wright");
}

// ---- 2. projection ---------------------------------------------------------
console.log("\nprojection — descriptor to verbs\n");
{
  const plan = residencyEntries(VESPER, { pos: [4, 0, -6] });
  ...
  check("the marker id is derived from the instance, so re-planting re-authors one entity",
    p.id === "residency-portos-vesper" && residencyEntries(VESPER).id === p.id);
  ...
  check("--no-lock leaves it loose", residencyEntries(VESPER, { lock: false }).entries.length === 2);
  ...
  const again = residencyEntries(VESPER, { skipSpawn: true });
...
{
  const log = [
    { seq: 10, ts: 1_000, actor: "vesper", verb: "say" },
    { seq: 11, ts: 2_000, actor: "vesper.wright", verb: "spawn" },
    { seq: 12, ts: 3_000, actor: "vesper.wright", verb: "comp" },
    { seq: 13, ts: 4_000, actor: "vesper.wright", verb: "spawn" },
    { seq: 14, ts: 5_000, actor: "somebody-else", verb: "spawn" },
  ];
  const traces = traceResidents(log, VESPER);
  const byName = Object.fromEntries(traces.map((t) => [t.wears, t]));
  check("the mind's acts are counted", byName.vesper.acts === 1 && byName.vesper.verbs.say === 1);
  check("an agent's acts are counted per verb", byName["vesper.wright"].acts === 3 && byName["vesper.wright"].verbs.spawn === 2);
  check("first and last are the entries' own timestamps",
    byName["vesper.wright"].first === 2_000 && byName["vesper.wright"].last === 4_000 && byName["vesper.wright"].lastSeq === 13);
  check("a declared agent that never acted is reported, not dropped",
    byName["vesper.scribe"].acts === 0 && traces.length === 3);
  check("a stranger's acts attach to nobody", !traces.some((t) => t.wears === "somebody-else"));

  const prose = describeTraces(traces, { now: 6_000 });
  check("the mind leads the prose", prose[0].startsWith("vesper "), prose[0]);
  ...
  const aliased = traceResidents([{ seq: 1, ts: 1, actor: "vw-7", verb: "say" }],
    { ...VESPER, agents: [{ id: "agent:vesper.wright@portos", as: "vw-7" }] });
...
{
  const line = describeResidency(normalizeResidency(VESPER).residency);
  check("the line names the instance, the mind and the roster size",
    line.includes("portos:vesper") && line.includes("Vesper") && line.includes("2 agents"), line);
  ...
  check("a malformed record still reads as somebody's marker",
    describeResidency({ instance: "portos:x" }).includes("nobody named"));
  check("a non-object bag reads as malformed", describeResidency(7).includes("malformed"));

  const begin = residencyLine("antra", "m1", residencyTransition(null, VESPER));
  check("a record beginning narrates once, naming the mind", begin!.includes("records Vesper's residency"));
  const end = residencyLine("antra", "m1", residencyTransition(VESPER, null));
  check("a withdrawal narrates as a withdrawal", end!.includes("withdraws"));
  const swap = residencyLine("antra", "m1", residencyTransition(VESPER, { ...VESPER, mind: { name: "Hesper" } }));
  check("a rededication names both minds", swap!.includes("Vesper") && swap!.includes("Hesper"));
  const tweak = residencyLine("antra", "m1", residencyTransition(VESPER, { ...VESPER, lore: "changed" }));
  check("an ordinary revision narrates as a revision", tweak!.includes("revises"));
  check("an identical re-author narrates nothing", residencyTransition(VESPER, { ...VESPER }) === null);
}
...
await new Promise<void>((resolve, reject) => {
  ...
  ws.onopen = () => ws.send(JSON.stringify({ type: "join", world: WORLD, id: "vesper.wright" }));
  ws.onmessage = (ev) => {
...
const plan = residencyEntries(VESPER, { pos: [2, 0, 3] }) as any;
...
ws.send(JSON.stringify({ type: "verb", verb: "comp", args: { id: "broken-marker", type: "residency", data: { instance: "portos:nobody" } } }));
await sleep(600);
...
check("...naming the instance that planted the marker", planted?.record?.instance === "portos:vesper");
...
const wright = planted?.traces?.find((t: any) => t.wears === "vesper.wright");
...
check("...and a declared agent that never acted is still listed",
  planted?.traces?.find((t: any) => t.wears === "vesper.scribe")?.acts === 0);
...
const { WorldAgent } = await import("../mcpl/agent.ts");
{
  ...
  ag.applyEntry({ verb: "spawn", args: { id: plan.id, lib: VESPER.marker.lib, pos: [2, 0, 3] }, ts: 1, seq: 1, actor: "vesper.wright" }, false);
  ag.applyEntry({ verb: "comp", args: { id: plan.id, type: "residency", data: plan.entries[1].args.data }, ts: 2, seq: 2, actor: "vesper.wright" }, false);
  const out = ag.look();
  check("look() names the residency on the entity that carries it",
    new RegExp(`\\[${plan.id}\\][^\\n]*residency: instance portos:vesper`).test(out),
  ...
  check("a live revision arrives as one ambient world-change line",
    events.length === 1 && events[0].text.includes("revises Vesper's residency"), JSON.stringify(events));
