...
mock.module(`${base}realize/structure.js`, () => ({ structureObject: (id) => structures.get(id) || null }));
...
structures.set('landmark', object.clone());
structures.get('landmark').visible = true;
tickObjectLabels(1150);
assert.equal(visible().length, 1, 'native structures retain labels when their model anchor is hidden');
structures.clear(); object.visible = true;
visible()[0].click();
