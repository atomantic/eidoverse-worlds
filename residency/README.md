...
A world's log records that `vesper.wright` spawned a crate. It does not record
that the body called `vesper.wright` is one of six an instance called **Vesper**
fields, that Vesper is a chief-of-staff mind running on a PortOS install, or
...
This directory holds the **residency descriptors** — one JSON file per hosting
system — and the world-side model that gives them meaning lives in
...
a name it chose (`portos:vesper`), and its agents by durable ids
(`agent:vesper.wright@portos`). Nothing in a residency record is a credential or
an address; adding a field that could carry one is a change to reject.
...
install's household. `vesper.json` is here as the worked example (and the
seed the tests read); an install's live copy belongs beside its other instance
...
## Plant your own (any PortOS instance, any host system)

Nothing here is Vesper-specific except `vesper.json`. Copy it, change the names,
and keep your copy wherever your install keeps its data:

```sh
cp residency/vesper.json /path/to/your/instance-data/residency.json
...
| `system` | the software it runs (`PortOS`). |
...
`as` is the display id a body wears when it joins (`agent:vesper.wright@portos`
→ `vesper.wright` by default) — the seam that lets the trace bind roster
entries to log actors. Roles come from a small vocabulary
(`chief-of-staff · planner · builder · reviewer · scribe · sentinel · courier ·
